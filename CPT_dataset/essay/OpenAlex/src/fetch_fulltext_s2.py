"""
FoodmoleGPT - Full Text Acquisition via Semantic Scholar + peS2o
================================================================
Phase 1: Map DOIs to Semantic Scholar Corpus IDs (batch API)
Phase 2: Stream peS2o from HuggingFace and extract matching full text

Usage:
    conda activate foodmole
    pip install datasets zstandard   # one-time install

    python src/fetch_fulltext_s2.py --phase 1   # DOI → Corpus ID mapping
    python src/fetch_fulltext_s2.py --phase 2   # Stream peS2o + filter
    python src/fetch_fulltext_s2.py --phase all  # Run both sequentially
    python src/fetch_fulltext_s2.py --test       # Test with 10 DOIs first
"""
...
S2_BATCH_SIZE = 500  # Max papers per batch request
S2_DELAY = 1.5  # Seconds between requests (rate limit is 1 req/s, add margin)

INPUT_CSV = Path("D:/FoodmoleGPT/data/cleaned/master_cleaned.csv")
OUTPUT_DIR = Path("D:/FoodmoleGPT/data/fulltext")
MAPPING_FILE = OUTPUT_DIR / "doi_to_corpusid.csv"
...
def load_progress() -> dict:
    """Load Phase 1 progress for resumability."""
...
def run_phase1(test_mode: bool = False):
    """Phase 1: Map DOIs to Semantic Scholar Corpus IDs."""
    print("\n" + "=" * 70)
    print("📋 PHASE 1: DOI → Semantic Scholar Corpus ID Mapping")
    print("=" * 70)
...
def run_phase2():
    """Phase 2: Stream peS2o .zst files from HuggingFace and extract matching full text."""
    print("\n" + "=" * 70)
    print("📖 PHASE 2: Streaming peS2o for Full Text Extraction")
    print("=" * 70)

    # Load Corpus ID mapping
    if not MAPPING_FILE.exists():
        print("   ❌ Phase 1 mapping file not found. Run Phase 1 first.")
        return

    print("   Loading Corpus ID mapping...")
    # Use on_bad_lines='skip' to handle rows where commas in URLs broke CSV format
    mapping_df = pd.read_csv(MAPPING_FILE, usecols=["s2_corpus_id"], on_bad_lines="skip")
    # Convert float -> int -> str to strip ".0" suffix (pandas reads as float64 due to NaN)
    corpus_ids = set()
    ...
    print(f"   Sample IDs: {list(corpus_ids)[:3]}")

    FULLTEXT_DIR.mkdir(parents=True, exist_ok=True)

    # Import huggingface_hub for direct file access
    try:
    ...
    # List peS2o v3 files (.zst compressed JSONL)
    # peS2o mixes s2orc (full text) and s2ag (abstract) in same files,
    # determined by the 'source' field in each JSON record
    print("   Listing peS2o v3 files on HuggingFace...")
    api = HfApi()
    all_files = api.list_repo_files("allenai/peS2o", repo_type="dataset")
    # v3 has 136 .zst files: data/v3/train-NNNN-of-0136.zst
    data_files = sorted([f for f in all_files if "data/v3/train-" in f and f.endswith(".zst")])

    if not data_files:
        # Fallback to v2 (20 .json.gz files)
        data_files = sorted([f for f in all_files if "data/v2/train-" in f])

    if not data_files:
        print("   ❌ No data files found. Available:")
    ...
    # Output file for full text (JSONL format)
    out_path = FULLTEXT_DIR / "food_science_fulltext.jsonl"
                ...
                with dctx.stream_reader(compressed) as reader:
                    ...
                    for line in text_stream:
                        ...
                        if doc_id in corpus_ids:
                            ...
                            # Only save full text documents with >500 chars
                            # v3 source format: 'pes2o/s2orc' (full text) or 'pes2o/s2ag' (abstract)
    ...
    # Save final stats
    stats = {
        "files_processed": len(s2orc_files),
        "scanned": scanned,
        "matched": matched,
...
def main():
    parser = argparse.ArgumentParser(description="FoodmoleGPT Full Text via S2 + peS2o")
    ...
    print("=" * 70)
    print("🚀 FoodmoleGPT - Full Text Acquisition")
    print("=" * 70)
    ...
    print("\n" + "=" * 70)
    print("✅ Full text acquisition pipeline complete!")
    print("=" * 70)
