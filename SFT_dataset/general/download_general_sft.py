"""
FoodmoleGPT — Download & Sample General SFT Data

Downloads and samples from 3 high-quality general SFT datasets,
converts to Alpaca format, and merges into a single file.

Target: ~30K general instruction pairs
  - smoltalk:           15K (diverse instruction following)
  - orca-agentinstruct:  10K (complex reasoning)
  - Nemotron:            5K  (instruction following + chat)

Usage:
    python download_general_sft.py
"""
...
# ══════════════════════════════════════════════════════════════════
# 1. SmolTalk — 15K from diverse subsets
# ══════════════════════════════════════════════════════════════════
def download_smoltalk():
    print("=" * 60)
    print("[1/3] SmolTalk — downloading & sampling 15K...")
    print("=" * 60)

    subset_plan = {
        "smol-magpie-ultra":   5000,   # Core general instruction following
        "numina-cot-100k":     2000,   # Math reasoning with CoT
        "openhermes-100k":     2000,   # Diverse QA
        "metamathqa-50k":      2000,   # Math
        "self-oss-instruct":   2000,   # Code/programming
...
def download_orca():
    print()
    print("=" * 60)
    print("[2/3] Orca AgentInstruct — downloading & sampling 10K...")
    print("=" * 60)
...
# ══════════════════════════════════════════════════════════════════
# 3. Nemotron — 5K from chat_if
# ══════════════════════════════════════════════════════════════════
def download_nemotron():
    print()
    print("=" * 60)
    print("[3/3] Nemotron — downloading & sampling 5K...")
    print("=" * 60)

    results = []
    print("  Loading chat_if...", end=" ", flush=True)
    try:
        ds = load_dataset(
        ...
        indices = sample_from_dataset(ds, 5000)
        count = 0
        for idx in indices:
            row = ds[idx]
            msgs = row.get("messages", [])
            alpaca = messages_to_alpaca(msgs)
            if alpaca:
                alpaca["general_source"] = "nemotron/chat_if"
                results.append(alpaca)
                count += 1
        print(f"✓ {count}/5000")
    except Exception as e:
    ...
    print(f"  Nemotron total: {len(results)}")
    return results
...
def main():
    ...
    nemotron_data = download_nemotron()

    # Merge all
    all_general = smoltalk_data + orca_data + nemotron_data
    ...
    alpaca_file = OUTPUT_DIR / "general_sft_30k.jsonl"
    ...
    full_file = OUTPUT_DIR / "general_sft_30k_full.jsonl"
    ...
    stats_file = OUTPUT_DIR / "general_sft_stats.json"
